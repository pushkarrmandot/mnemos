...
/**
 * Persistence plumbing shared by the persisted slices.
 *
 * **Deliberate divergence from the spec:** the design calls for the Tauri
 * `store` plugin writing `~/Mnemos/state/ui.json`. That plugin is not a
 * dependency yet and its API is async, which the
 ...
 * This module therefore persists to `localStorage` instead. Swapping the
