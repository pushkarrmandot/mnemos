...
/**
 * Home's "Your to-dos" (`02_DASHBOARD_AND_NAV.md`'s YOUR TO-DOS section) —
 ...
 * tagged `assignee_hint = "You"` show up here. A brand-new user will see
 * fewer than what's "really" theirs until the assignee picker gets used a
 * few times — accepted, not treated as a bug.
 */
export function YourToDosCard() {
  ...
  // `assignee_hint = "You"`, so a two-step create-then-assign left a window
  ...
  const create = useMutation({
    mutationFn: (text: string) =>
      commands.conversation.createStandaloneActionItem(null, text, "You"),
    onSuccess: invalidate,
